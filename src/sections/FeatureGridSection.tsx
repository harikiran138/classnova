...
export const FeatureGridSection = () => {
  return (
    <section id="features" className="section-padding bg-slate-50 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-orange-100/50 rounded-full blur-[120px] translate-y-1/2" />

      {/* Massive Watermark Heading */}
      <div className="absolute top-20 left-0 w-full overflow-hidden pointer-events-none opacity-[0.03]">
        <div className="text-[20vw] font-black text-slate-900 leading-none whitespace-nowrap text-center uppercase tracking-tighter">
          Key Features
        </div>
      </div>
      ...
      <div className="section-shell relative z-10">
        <div className="mb-20 max-w-3xl">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-600 text-sm font-bold uppercase tracking-wider mb-8"
          >
          ...
          <motion.h2
            ...
            className="text-5xl sm:text-6xl md:text-7xl font-bold text-slate-900 mb-8 tracking-tight"
          >
            Built for the <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[320px]">
          {features.map((feature, idx) => (
            <motion.div
              ...
              className={`${feature.colSpan} group relative rounded-[40px] p-10 overflow-hidden border ${feature.bg.includes('bg-white') ? 'border-slate-200 hover:border-blue-200 hover:shadow-xl' : 'border-white/10 hover:border-white/20'} transition-all duration-500 ${feature.bg}`}
            >
              {/* Hover Glow Effect */}
              <div className={`absolute inset-0 bg-gradient-to-br ${feature.bg.includes('bg-white') ? 'from-blue-50' : 'from-white/5'} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                <div>
                  <div className={`w-14 h-14 rounded-2xl ${feature.bg.includes('bg-white') ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-white/10 backdrop-blur-md border-white/20 text-white'} flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-300`}>
                    <feature.icon className="w-7 h-7" />
                  </div>
                  <h3 className={`text-3xl font-bold ${feature.text} mb-4`}>{feature.title}</h3>
                  <p className={`text-lg ${feature.bg.includes('bg-white') ? 'text-slate-600' : 'text-white/70'} leading-relaxed font-medium`}>
                    {feature.desc}
                  </p>
                </div>

                <div className={`flex items-center gap-2 text-sm font-bold ${feature.bg.includes('bg-white') ? 'text-blue-600' : 'text-white/50 group-hover:text-white'} transition-colors uppercase tracking-wider`}>
