...
def start_tunnel(ip, user, password):
    print(f"Starting UPDATED SSH Tunnel (V4) to {user}@{ip}...")
    
    # Forward:
    # 6081 (Local) -> 6081 (Remote TightVNC Websockify)
    # 3000 (Local) -> 3000 (Remote React App)
    # 5001 (Local) -> 5001 (Remote Backend API)
    cmd = f"ssh -L 6081:localhost:6081 -L 3000:localhost:3000 -L 5001:localhost:5001 -N {user}@{ip}"
    ...
    try:
        ...
        if process.poll() is not None:
             ...
             print("Forwarding:")
             print("- localhost:6081 -> Remote TightVNC")
             print("- localhost:3000 -> Remote React App")
             print("- localhost:5001 -> Remote Backend API")
